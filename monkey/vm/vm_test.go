...
func TestFirstClassFunctions(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `let returnOne = fn() { 1; };
					let returnOneReturner = fn() { returnOne; };
					returnOneReturner()();`,
			expected: 1,
		},
		{
			input: `let returnOneReturner = fn() {
				let returnOne = fn() { 1; };
				returnOne;
			};
			returnOneReturner()();`,
...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case string:
		...
		if err != nil {
			t.Errorf("test string object failed:\n%s", err)
		}
	case []int:
		...
		if len(array.Elements) != len(expected) {
			t.Errorf("wrong element count. want %d, got %d", len(expected), len(array.Elements))
			return
		}
	...
	case map[object.HashKey]int64:
		...
		if len(hash.Pairs) != len(expected) {
			t.Errorf("has has wrong pair count. expected %d, got %d", len(expected), len(hash.Pairs))
			return
		}
